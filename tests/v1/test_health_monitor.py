...
class TestHealthMonitor:
    ...
    def test_run_quick_checks_updates_health_status(self, monitor):
        """Test that run_quick_checks updates health status immediately."""
        toggle = SimpleHealthCheck(healthy=True)
        monitor._health_checks = [toggle]
...
class TestRemoteBackendHealthCheck:
    ...
    def test_set_trigger_callback(self, mock_backend):
        """Test that set_trigger_callback registers callback with backend."""
    ...
    def test_quick_check_passes_when_below_threshold(self, mock_backend):
        """Test quick_check returns True when failures below threshold."""
        mock_backend.get_interval_remote_failed_count.return_value = 0
    ...
    def test_quick_check_fails_when_exceeds_threshold(self, mock_backend):
        """Test quick_check returns False when failures exceed threshold."""
        mock_backend.get_interval_remote_failed_count.return_value = 10
        mock_backend.config.get_extra_config_value.return_value = 5
    ...
    def test_quick_check_returns_false_when_in_failure_recovery(self, mock_backend):
