...
def create_polling_scheduler_adapter(
    server_url: str, zmq_context: zmq.Context, vllm_config: VllmConfig
) -> LMCacheMPPollingSchedulerAdapter:
    """Create a LMCacheMPPollingSchedulerAdapter whose check_lookup_result
    blocks until the prefetch job is complete, avoiding blocking the
    LMCache server's worker threads.
    """
    world_size, kv_rank = extract_world_size_and_kv_rank(
        vllm_config.parallel_config.world_size,
        vllm_config.parallel_config.rank,
