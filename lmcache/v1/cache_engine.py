...
class LMCacheEngine:
    ...
    def _process_tokens_internal(
        ...
        This function processes the given tokens and mask to identify and retrieve
        cached key-value pairs. It populates the provided lists with the results.

        Side effects on input parameters:
          - reordered_keys: Appends keys for the retrieved cache entries
          - reordered_memory_objs: Appends the retrieved memory objects
          - reordered_starts: Appends the start indices of the token spans
          - reordered_ends: Appends the end indices of the token spans
          - ret_mask: Updates the mask in-place to mark which tokens have been cached

        Args:
            tokens: Input tokens to process
            mask: Mask indicating valid token positions
            reordered_keys: List to collect keys of retrieved cache entries
            reordered_memory_objs: List to collect retrieved memory objects
            reordered_starts: List to collect start indices of token spans
            reordered_ends: List to collect end indices of token spans
