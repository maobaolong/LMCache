    ...
    try:
        ...
        if config_key == "extra_config" and isinstance(value, dict):
            ...
            if should_override:
                ...
                logger.info("Overridden extra_config: %s", value)
                return True
            ...
            if current_value is not None and isinstance(current_value, dict):
                ...
                logger.info("Merged extra_config: %s", merged_value)
                return True
            # If current value is None or not a dict, just set the new value
            setattr(config, config_key, value)
            logger.info("Set extra_config: %s", value)
            return True
        ...
        setattr(config, config_key, value)
        logger.info("Set config item '%s' with value %s", config_key, value)
        return True
