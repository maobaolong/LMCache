...
async def kvcache_check(
    ...
    Args:
        ...
        instance_id: GPU context ID (default: first registered).
    ...
    if instance_id is None:
        if not gpu_ctxs:
            return JSONResponse(
                status_code=404,
                content={"error": "no GPU context registered"},
            )
        instance_id = next(iter(gpu_ctxs))
    ...
    if slot_indices is None:
        return JSONResponse(
            status_code=400,
            content={
                "error": "failed to parse slot_mapping",
            },
        )
