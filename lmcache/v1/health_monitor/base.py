...
class HealthCheck(ABC):
    ...
    def set_trigger_callback(  # noqa: B027
        ...
        health checks to register callbacks with their monitored components.

        Override this method to implement custom callback registration logic.
...
class HealthMonitor:
    ...
    def run_quick_checks(self) -> None:
        ...
        This method is called from external components (e.g., RemoteBackend)
        when a failure occurs. It runs lightweight quick_check() on all
        health checks. If any quick check fails, the system is immediately
        marked as unhealthy.

        Quick checks are designed to be fast (no network I/O) and can be
        called frequently. Full checks (ping, etc.) only run on the regular
        interval to avoid excessive overhead.
        ...
        for check in self._health_checks:
            ...
            try:
                ...
                logger.error("Quick check %s raised exception: %s", check_name, e)
                is_healthy = False

            if not is_healthy:
                # Quick check failed
                logger.warning("Quick health check failed: %s", check_name)
    ...
    def _run_loop(self) -> None:
        """Main monitoring loop for full health checks."""
        logger.info(
            "Starting health monitor loop with interval %ss",
            self._ping_interval,
        )
